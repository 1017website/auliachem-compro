...
class AdvancedFeaturesTest extends TestCase
{
    ...
    protected function setUp(): void { parent::setUp(); app(CompanyPage::class)->import(); }
    private function admin(bool $active=true): User { $user=User::factory()->create(['is_active'=>$active]);$user->is_admin=true;$user->save();return $user; }

    public function test_product_catalog_is_managed_and_only_published_products_render(): void
    {
        Storage::fake('public'); $admin=$this->admin();
        ...
        $product=Product::firstOrFail(); Storage::disk('public')->assertExists($product->image_path);
        $this->get('/?lang=en')->assertOk()->assertSee('Sodium Chloride')->assertSee('AC-001');
        $product->update(['is_published'=>false]); $this->get('/')->assertDontSee('AC-001');
    }
    ...
    {
        $inactive=$this->admin(false); $this->actingAs($inactive)->get('/admin')->assertForbidden();
        $active=$this->admin(); $this->actingAs($active)->get('/admin/profile')->assertOk()->assertSee('Riwayat login');
        $this->post('/admin/profile/two-factor')->assertRedirect(); $this->assertNotNull(session('two_factor_setup'));
    }
    ...
    {
        Storage::fake('public'); $field=ContentField::where('type','image')->where('key','!=','brand_favicon')->firstOrFail();$section=$field->section;
        ...
        foreach(ContentField::where('section',$section)->get() as $item){$payload['fields'][$item->id]=$item->values;$payload['version'][$item->id]=hash('sha256',json_encode($item->values));}
        $this->actingAs($this->admin())->put('/admin/content',$payload)->assertRedirect()->assertSessionHasNoErrors();
        $field->refresh();$this->assertSame('Facility photo',$field->alt_values['en']);$this->assertSame('top',$field->image_position);$this->assertStringStartsWith('/storage/cms/mobile/',$field->mobile_value);
    }
}
